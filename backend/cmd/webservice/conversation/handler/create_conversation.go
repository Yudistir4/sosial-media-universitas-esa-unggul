...
func (h *conversationHandler) CreateConversation(service service.ConversationService) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// var req dto.
		req := new(dto.CreateConversationReq)
		err := c.Bind(req)
		req.UserIDs = append(req.UserIDs, loggedInUser.UserID)
		if err != nil {
		...
		conversation, err := service.CreateConversation(*req)
		if err != nil {
			return httputils.ErrorResponse(c, httputils.ErrorResponseParams{
				Err: err,
			})
