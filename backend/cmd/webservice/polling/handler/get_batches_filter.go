...
func (h *pollingHandler) GetBatchesFilter(service service.PollingService) echo.HandlerFunc {
	return func(c echo.Context) error {

		// Binding data and assign userID
		var req dto.GetBatchesFilterReq
		err := c.Bind(&req)
		...
		users, err := service.GetBatchesFilter(req.FacultyID, req.StudyProgramID)
		if err != nil {
			return httputils.ErrorResponse(c, httputils.ErrorResponseParams{
				Err: err,
			})
		...
		return httputils.SuccessResponse(c, httputils.SuccessResponseParams{
			Data:    users,
			Message: "Get batches filter successfully",
		})
