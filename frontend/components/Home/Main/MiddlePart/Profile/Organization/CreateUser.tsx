...
interface ICreateUserOrganizationFormProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const CreateUserOrganizationForm: React.FunctionComponent<
  ICreateUserOrganizationFormProps
> = ({ isOpen, onClose }) => {
  ...
  const {
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Create account organization success',
        status: 'success',
      });
      reset();
      onClose();
  ...
  const onSubmit = (data: CreateOrganizationForm) => mutate(data);
  ...
  return (
    <div>
      ...
      >
        <ModalOverlay />
        <ModalContent mx={2}>
          <ModalHeader>Create Account Organization</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <form
                ...
                <Alert status="error" className="items-center">
                  ...
                  <Flex className="flex-wrap">
                    <AlertTitle className="shrink-0">
                      Create account organization failed!
                    </AlertTitle>
      ...
      >
        ...
        <AlertDialogContent mx={2}>
          <AlertDialogHeader>Discard account organization?</AlertDialogHeader>
          <AlertDialogCloseButton />
          <AlertDialogBody>
...
export default CreateUserOrganizationForm;
