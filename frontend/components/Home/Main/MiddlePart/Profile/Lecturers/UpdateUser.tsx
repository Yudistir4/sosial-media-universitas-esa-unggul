...
const UpdateUser: React.FunctionComponent<IUpdateUserProps> = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      faculty_id: user.lecturer.faculty.id,
      study_program_id: user.lecturer.study_program.id,
    },
  });
  const {
    mutate,
    ...
    mutationFn: async (data: UpdateLecturerForm) => {
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Update account lecturer success',
        status: 'success',
      });
      reset();
      onClose();
  ...
  React.useEffect(() => {
    if (user.lecturer.faculty.id === faculty_id) return;
  ...
  const onSubmit = (data: UpdateLecturerForm) => mutate(data);
  ...
  return (
    <>
      ...
      >
        <ModalOverlay />
        <ModalContent mx={2}>
          <ModalHeader>Update Account lecturer</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <form
                ...
                <Alert status="error" className="items-center">
                  ...
                  <Flex className="flex-wrap">
                    <AlertTitle className="shrink-0">
                      Update account lecturer failed!
                    </AlertTitle>
